...
This pipeline prepares Dreem recordings for downstream signal analysis in **Luna**, a command-line toolkit developed for large-scale sleep data analysis. Luna allows flexible, scriptable processing of EDF files and annotations, enabling efficient extraction of per-epoch features, signal quality metrics, and spectral measures.
...
5.  **Save the file.** This manifest will be used as the authoritative source for renaming and copying files into their final locations in Step 3.
...
## Filtering for EDFs ≥ 4 Hours and Generating the Sample List
...
#### Initial Command File: `dreem_initial_db.txt`
...
Initializes fixed 30-second epochs across the entire EDF file. Most Luna commands, including artifact detection, masking, and spectral analysis, require data to be epoched. Epochs act as time-aligned units for signal comparison and summarization.

------------------------------------------------------------------------

##### `SIGNALS keep=EEG_F7_O1,EEG_F8_O2,EEG_F8_F7,EEG_F8_O1,EEG_F7_O2`

Limits analysis to a specified set of EEG channels. Channels Accelero_Norm, Positiongram, Respiration_x, Respiration_y, and Respiration_z are dropped.

------------------------------------------------------------------------

##### `MASK ifnot=N1,N2,N3,R,W`

Applies a sleep-stage filter, keeping only epochs scored as NREM stages (N1–N3), REM, or Wake. Epochs scored as movement (MT) or unknown are excluded to minimize artifact contamination in downstream analyses.

------------------------------------------------------------------------

##### `RESTRUCTURE`

Applies any existing masks (e.g., from `MASK` and `SIGNALS keep`) and permanently removes the masked epochs or channels from the internal EDF. This ensures all subsequent metrics and outputs are based only on intentionally retained data.

------------------------------------------------------------------------

##### `FILTER bandpass=0.3,35`

Applies a bandpass filter using a linear-phase FIR design to retain frequencies between 0.3 and 35 Hz. This removes slow drift and high-frequency noise, targeting the typical EEG range used in sleep studies.

##### `ripple=0.01` and `tw=0.5`

These parameters refine the filter design. `ripple=0.01` sets the acceptable amplitude variation in the stopband (\~–40 dB attenuation), and `tw=0.5` sets the transition width between passband and stopband to 0.5 Hz, balancing sharpness and stability.

------------------------------------------------------------------------

##### `CHEP`

These commands perform Hjorth-parameter-based artifact detection.

-   `CHEP-MASK ep-th=3`

    -   Identifies *within-channel, between-epoch* outliers using Hjorth parameters. A two-pass SD-based method flags cheps (channel+epoch pairs) where any Hjorth metric deviates beyond 3 SD from the per-channel distribution across epochs. These cheps are added to the internal CHEP mask but are not yet excluded from analysis.

-   `CHEP epochs dump`

    -   Sets a channel-agnostic *epoch-level mask* based on the current CHEP mask. An epoch is masked if **any** of its channels were flagged as outliers. This mask is then saved (via `dump`) and becomes available for operations like RESTRUCTURE. This enables synchronization between channel-based artifact detection and epoch-level exclusion.

-   `CHEP-MASK ch-th=2`

    -   Performs *within-epoch, between-channel* outlier detection. For each epoch, a chep is flagged if any Hjorth parameter for that channel deviates more than 2 SD from the distribution of all channels in that same epoch. These are added to the CHEP mask. This step complements the `ep-th` approach by detecting spatially inconsistent artifacts.

-   `CHEP channels=0.3 dump`

    -   Evaluates the existing CHEP mask and flags entire channels as bad if **\>30%** of their cheps were previously flagged. These channels are then fully masked for all epochs. The `dump` option saves this information to the output stream for reproducibility and inspection. This helps exclude persistently noisy channels from further processing.

------------------------------------------------------------------------

##### `RESTRUCTURE` (second call)

Applies the updated epoch-level mask (set by `CHEP epochs`) and finalizes the exclusion of both poor-quality epochs and persistently noisy channels. This ensures clean, artifact-minimized data for subsequent feature extraction.

------------------------------------------------------------------------

##### `SIGSTATS epoch`

Computes signal statistics at two levels:

-   **Overall per-channel (CH):** calculates Hjorth parameters (H1–activity, H2–mobility, H3–complexity) and other metrics like RMS, min, max, skewness across the full recording.

-   **Per-channel per-epoch (CH x E):** repeats H1–H3 plus RMS (**Root Mean Square –** a standard measure of signal amplitude that reflects the average power of a waveform) for each 30‑second epoch (excluding masked epochs), enabling time‑resolved signal quality checks.

------------------------------------------------------------------------

##### `PSD spectrum`

Computes the power spectral density using Welch's method, giving per-epoch frequency-domain summaries for each signal. This is essential for analyzing sleep depth, arousals, and frequency-specific phenomena like slow-wave activity or alpha suppression.
...
#### Script: `build_initial_dreem_db.sh`
...
luna /Dreem/data/processed/sample_list/min_4hr_edf.lst \
     -o /Dreem/data/processed/luna_outputs/dreem_initial.db \
     < /Dreem/scripts/luna_cmds/dreem_initial_db.txt
```
...
## Assigning Group and Session (visit + group) Metadata to Participants

To enable group-based statistical comparisons (e.g., evaluating power spectral density across AE severity), we created a reproducible metadata file (`id_group_map.csv`) linking each EDF file to participant-level and session-level attributes.

#### Why this step is important

Luna does not natively support internal tagging of recordings with metadata like group membership, visit, or night. These values are critical for stratified downstream analysis, such as comparing sleep architecture or CHEP-filtered PSD metrics across disease severity.

By producing a clean and standardized metadata table, we ensure that all subsequent analyses can accurately align outputs (e.g., from `sigstats`, `PSD`, or `CHEP`) to participant characteristics.

#### What the script does

-   Loads the cleaned REDCap export (`redcap_clean.csv`), which contains `subject_id` and full-text group labels (e.g., `"Group 1 (immuno-modulatory therapy)"`).

-   Maps these labels to simplified categories: `"severe AE"`, `"mild AE"`, and `"healthy controls"`.

-   Loads the 4-hour filtered sample list (`min_4hr_edf.lst`), ensuring only valid EDFs used in the Luna database are included.

-   Parses the standardized IDs (e.g., `MS01_V1_N1`) to extract:
...
-   Merges with group info from REDCap.

-   Drops any unmatched entries (e.g., REDCap participants without recordings or vice versa).

-   Outputs a five-column metadata file with:

    -   `standard_id`: the unique filename identifier (used across Luna and outputs)

    -   `subject_id`: participant ID (e.g., `MS03`)

    -   `group`: categorical label (`severe AE`, `mild AE`, or `healthy controls`)

    -   `visit`: visit label (e.g., `V1`)

    -   `night`: night label (e.g., `N3`)

#### Output

The resulting `id_group_map.csv` ensures consistent metadata joins for downstream stratification in R and integration into `.db` analyses (e.g.

### Script: `generate_metadata_from_redcap.R`
