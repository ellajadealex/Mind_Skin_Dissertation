...
## Assigning Group Labels to Participants

To support group-based statistical analysis (e.g., comparing power spectral density across eczema severity groups), we created a reproducible metadata mapping of participant IDs to their assigned group (1–3).

### Why this step is important

Luna itself does not support tagging subjects with custom metadata (e.g., group or age) internally. Instead, downstream tools like R are used to merge this information during analysis. To enable this, we generate a clean, standardized `id_group_map.csv` that links each subject-night file in our sample to the correct group.

We limit this mapping to only the EDF files passing our ≥4-hour sleep threshold, ensuring that metadata aligns exactly with the files used in database creation and analysis (`dreem_chep_psd_sigstats.db`).

### What the script does

-   Reads a cleaned REDCap export (`redcap_clean.csv`) containing subject IDs and group assignments.

-   Loads the 4-hour filtered sample list (`min_4hr_edf.lst`) used to build the Luna database.

-   Extracts the standardized IDs (e.g., `MS01_V1_N1`) from full EDF paths.

-   Merges these IDs with group information from REDCap.

-   Drops unmatched rows (e.g., REDCap entries with no EDF or vice versa).

-   Outputs a two-column CSV (`id_group_map.csv`) containing:

    -   `standard_id`: the unique ID used in Luna and `.db` files

    -   `group`: assigned patient group (1, 2, or 3)

This file can then be used in R for stratified analysis or plotting by merging with outputs like PSD or sigstats. It is integrated into the db later on in analysis.
